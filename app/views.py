...
def index():
    ...
    if request.method == "POST":
        ...
        original_image = Image.open(
            os.path.join(app.config["EXISTNG_FILE"], "image.jpg")
        ).resize((250, 160))
        ...
        cv2.imwrite(
            os.path.join(app.config["GENERATED_FILE"], "image_thresh.jpg"), thresh
        )
        return render_template(
            "index.html", pred=str(round(score * 100, 2)) + "%" + " correct"
        )
